...
| R2 Sparse only | 0.6368 | 0.0000 | 1.0000 | 0.0045 | 0.0000 | 0.5456 | 0.0022 | 0.3283 | 4.5 |
...
| R2 Sparse only | 0.5781 | 0.0000 | 1.0000 | 0.0098 | 0.0000 | 0.5260 | 0.0049 | 0.3176 |
...
| R2 Sparse only | 0.5365 | 0.0000 | 1.0000 | 0.0000 | 0.0000 | 0.5122 | 0.0000 | 0.3073 |
...
| R2 Sparse only | 0.7286 | 0.0000 | 1.0000 | 0.0043 | 0.0000 | 0.5762 | 0.0022 | 0.3466 |
...
| R2 Sparse only | 0.6905 | 0.0000 | 1.0000 | 0.0043 | 0.0000 | 0.5635 | 0.0022 | 0.3390 |
...
This pattern holds in every category (1–4), not just on average — Claim Coverage and Attribution Faithfulness both crash to near-zero for G1 across all four categories, while the 3 shared metrics dip only modestly. See `t2_per_metric_table.md` for the full per-category breakdown.
...
## Second finding: G1 is not the largest drop on shared metrics — R2 is

| Variant | mean3 (shared) | Δ vs full | mean5 | Δ vs full |
|---|---|---|---|---|
| G1 Flat chunks | 0.7861 | -0.0711 (-8.3%) | 0.4845 | -0.3138 (-39.3%) |
| R2 Sparse only | 0.5456 | -0.3116 (-36.4%) | 0.3283 | -0.4700 (-58.9%) |

On `mean_5`, G1 (−39.3%) and R2 (−58.9%) both look severe, in that order. But on the 3 shared metrics — the ones actually comparable to SQuAI and to what a reviewer would recognize as standard RAG quality — **R2's drop (−36.4%) is over 4x larger than G1's (−8.3%)**. Broken/degenerate retrieval (sparse-only) is the more damaging ablation on the metrics that travel outside this paper; the evidence-graph ablation's severity is concentrated in the metrics this paper itself defined.
...
2. Keep the evidence-graph claim, but scope it accurately: *"removing the evidence graph causes a measurable drop on standard RAG metrics (−8.3%) and a much larger drop on claim/attribution-specific metrics we introduce (−94%+), which is expected since those metrics measure graph-derived structure directly."*
3. Don't let G1 stand alone as "the largest drop" — R2 (sparse-only) is the largest drop on the metrics shared with SQuAI, and that finding is at least as interesting (hybrid retrieval is essential; dense signal alone recovers most quality, sparse alone collapses it).
