...
class _QdrantConnection:
    ...
    url:        Optional[str] = field(
        # HPC / local Docker: leave None → use host:port (Singularity or localhost).
        # PROD=true or pip users without a local instance: fall back to hosted VM.
        # Always overridable via QDRANT_URL env var.
        default_factory=lambda: os.getenv("QDRANT_URL") or (
            "http://141.76.56.250:6333"
            if os.getenv("PROD", "false").lower() in ("1", "true", "yes")
            else None
        )
    )
    timeout:    int = field(
